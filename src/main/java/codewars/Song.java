...
public class Song {
    ...
    private List<String> alreadyListenedPeople = new ArrayList<>();
    ...
    public int howMany(List<String> arrayPeople){
        int count = 0;
        for (String human : arrayPeople){
            human = human.toLowerCase();
            if (!alreadyListenedPeople.contains(human)){
              count++;
              alreadyListenedPeople.add(human);
            }
        }
        System.out.println(alreadyListenedPeople);
        return count;
    }
